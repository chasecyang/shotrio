...
[x]去掉后台任务，这个功能太冗余了
[]还是要让Agent懂首尾帧中间帧，懂多个视频凑成一个故事，懂参考素材
[]让agent知道项目是要竖屏的还是横屏

[]让agent有向用户提问的能力？

[]截图功能不好用，素材库要刷新才展示新截的图，而且刷新了展示了截图，图据让还是裂的
...
[x]素材管理组件本地化 (asset-detail-view, media-viewer, image-upload)
[x]素材生成和预览组件本地化
[x]项目设置和验证本地化
[x]优化导出视频的ui，目前在后台任务中，体验较差
[x]视频缩略图有点糊
...
[x]要让agent知晓素材被废弃了/star了
[x]在检查资产的时候点击生成，依旧需要很长的时间才会结束响应，为什么？
[x]agent目前没有创建新的cut的能力 - 已添加 create_cut function
[x]agent的function描述里面，不再以时间轴为主要名词，应该改成剪辑/cut - 已更新所有相关描述
