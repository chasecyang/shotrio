...
export async function generateSceneImage(params: {
  ...
  try {
    const input: TextToImageInput = {
      ...
      aspect_ratio: params.aspectRatio ?? "16:9", // 场景默认使用 16:9
 ...
 * 批量生成场景图像（使用队列）
 */
export async function queueSceneImageGeneration(params: {
  description: string;
  aspectRatio?: AspectRatio;
  resolution?: Resolution;
  numImages?: number;
  ...
  try {
    ...
    const { request_id } = await queueTextToImagePro(input);
    ...
    console.error("提交场景图像生成队列失败:", error);
    return {
      success: false,
      error: error instanceof Error ? error.message : "提交队列失败",
    };
  }
}
